...
**Note on the tied scores themselves:** even after this fix, residents may still show identical/near-identical Ongoing KPI scores this early in a new academic year if `mmPct`/`tPct` are both defaulting to 100% (no attendance marked yet for the current AY) and `qAvg` is the same across residents (e.g. only one shared quiz scored similarly). That's a data-completeness artifact of the season, not a code bug — worth re-checking once more MM/Teaching sessions and quizzes are recorded for AY 2025-26.
