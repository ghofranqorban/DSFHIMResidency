...
| `fix_kpi_scores_read_rls.sql` | ⚠️ NOT yet run | Fixes Performance Report leaderboard showing different rankings/percentages depending on who's logged in (reported 18 Jul 2026 — Deema saw a completely different top-5 than the PD view). Root cause: `kpi_scores` SELECT RLS (`add_kpi_scores.sql`) restricted reads to PD/chief/own-mentee/own-row only, but `calcKPI()` reads the globally-cached `KPI_SCORES` for every resident to build the cohort-wide leaderboard — so a resident's session only ever saw their own Committee Score + yearly bonus flags, and everyone else's silently defaulted to 0/false in that session's score math. Widens SELECT to all authenticated (write policy unchanged), same pattern as `rotations`/`mm_attendance`/`teaching_attendance`/`quiz_scores`. **Must run in Supabase SQL Editor.**
...
**Fix written, not yet run:** `supabase/fix_kpi_scores_read_rls.sql` — widens `kpi_scores` SELECT to all authenticated users (`using (true)`), same pattern as `rotations`/`mm_attendance`/`teaching_attendance`/`quiz_scores`. Write policy (`kpi_write`, PD/chief/assigned mentor only) is unchanged. **⚠️ Must be run in Supabase SQL Editor** — leaderboard will keep varying by viewer until then.

**Still TODO:** Run `fix_kpi_scores_read_rls.sql` in Supabase SQL Editor, then spot-check the Performance Report leaderboard from both a PD login and a resident login to confirm identical rankings. Carried forward unchanged: historical KPI data entry; On-Call Statistics tab visibility; `renderKPI()`/`renderAdminProfile()` splits; new AY 2026-27 master rota setup; stale untracked `perf_report_preview.html`.
