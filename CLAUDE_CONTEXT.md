...
| `add_schedule_draft_publish.sql` | ⚠️ NOT run yet | Adds `published boolean not null default false` to `mm_sessions`/`teaching_sessions`, updates SELECT RLS to `published OR is_pd_or_chief() OR has_priv(...)`, and backfills all *existing* rows to `published=true` (so nothing currently visible disappears on run). **Run this before the Publish/Unpublish buttons actually restrict draft visibility** — until run, the new `published` column doesn't exist so the client-side badge/toggle will error or no-op. |
